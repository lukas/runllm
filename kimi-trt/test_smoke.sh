#!/usr/bin/env bash
# Smoke test: verify TensorRT-LLM responds to a chat completion request.
...
if not c:
    sys.exit(1)
msg = c[0].get('message', {})
t = msg.get('content', '') or msg.get('reasoning_content', '') or msg.get('reasoning', '')
print(t)
" 2>/dev/null)
